...
public class CheckinDao {
	...
	public CheckinDto checkinIn(String userid) throws SQLException{
		...
		String sql = "INSERT INTO CHECKINLIST (STUNUM, CHECKDATE, CHECKIN) VALUES ((SELECT USERNUM FROM MEMBER WHERE USERID=?),(SELECT TO_CHAR(SYSDATE,'YYYYMMDD')FROM DUAL),(SELECT TO_CHAR(SYSDATE,'HH24MISS')FROM DUAL))";
		String sql2 = "SELECT CHECKIN FROM CHECKINLIST WHERE STUNUM=(SELECT USERNUM FROM MEMBER WHERE USERID=?) AND CHECKDATE=(SELECT TO_CHAR(SYSDATE,'YYYYMMDD')FROM DUAL)";
	...
	public CheckinDto checkinOut(String userid) throws SQLException{
		...
		String sql = "SELECT CHECKIN FROM CHECKINLIST WHERE CHECKDATE='" + date + "'";
		String sql2 = "UPDATE CHECKINLIST SET CHECKOUT=(SELECT TO_CHAR(SYSDATE,'HH24MISS')FROM DUAL) WHERE STUNUM=(SELECT USERNUM FROM MEMBER WHERE USERID=?) AND CHECKDATE='" + date + "'";
		String sql3 = "SELECT CHECKOUT FROM CHECKINLIST WHERE STUNUM=(SELECT USERNUM FROM MEMBER WHERE USERID=?) AND CHECKDATE='" + date + "'";
